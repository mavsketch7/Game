...
function onDataHost(conn, d) {
        if (d.t === "join") {
          ...
          if (s) {
            ...
            if (d.gremio !== undefined) s.gremio = d.gremio;
            construirMenu();
          }
        } else if (d.t === "input") {
...
export function enviarRolPropio(rol, listo, nombre, gremio) {
        if (NET.modo !== "cliente" || !NET.conn || !NET.conn.open) return;
        NET.conn.send({ t: "rol", rol, listo, nombre, gremio });
      }
...
function serPlayer(p) {
        ...
        return {
          ...
          reviveT: p.reviveT,
          swingT: p.swingT,
          comboPaso: p.comboPaso,
          comboAnimT: p.comboAnimT,
          comboDur: p.comboDur,
          atkEspecial: p.atkEspecial,
          golpeT: p.golpeT,
          anim: p.anim,
          forma: p.forma,
          escudo: p.escudo,
